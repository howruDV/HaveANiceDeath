...
void UICollider2D::render_update()
{
	...
	static int item_currentType = (int)GetTargetObject()->Collider2D()->GetType();
	bool bAbsolute = GetTargetObject()->Collider2D()->IsAbsolute();

	ImGui::SeparatorText("Transform");
	ImGui::Text("Offset   "); ImGui::SameLine();  ImGui::DragFloat3("##Collider Offset Position", vPos);
	ImGui::Text("Scale    "); ImGui::SameLine(); ImGui::DragFloat3("##Collider Offset Scale", vScale);
	ImGui::Text("Scale Absolute"); ImGui::SameLine(); ImGui::Checkbox("##Collider Offset Absolute", &bAbsolute);
